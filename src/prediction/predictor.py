...
class AQIPredictor:
    ...
    def predict_next_3_days(self):
        ...
        for hour_offset in range(1, 73):
            ...
            # THE FIX: Instead of copying the immediate last hour (-1) which causes a flatline,
            # we copy the exact same hour from the previous day (-24). 
            # This simulates a realistic daily cycle for weather and baseline pollutants.
            ...
            # THE FIX: Calculate interaction term based on the current row's diurnal values
            col = 'pm2_5_x_wind_speed'
            if col in new_row.index:
                new_row[col] = new_row['pm2_5'] * new_row['wind_speed']
