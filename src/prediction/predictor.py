...
class AQIPredictor:
    ...
    def predict_next_3_days(self):
        ...
        # Keep 72 rows so lag-48 has valid lookups from real data at hour 1
        history = self.get_latest_features().tail(72).copy().reset_index(drop=True)
        ...
        # aqi_values tracks the rolling AQI window (real + predicted) so lag
        # and rolling features always read correctly updated values
        aqi_values = list(history['aqi'].values)
        ...
        for hour_offset in range(1, 73):
            ...
            # Copy same hour from previous day for realistic diurnal baseline
            # (weather/pollutant base values follow a daily cycle)
            new_row = history.iloc[-24].copy()
            ...
            # --- Lag features: read from aqi_values (real + predicted) ---
            lags = [1, 3, 6, 12, 24, 48]
            for lag in lags:
                col = f'aqi_lag_{lag}h'
                if col in new_row.index and lag <= len(aqi_values):
                    new_row[col] = aqi_values[-lag]

            # --- Rolling features: computed from aqi_values ---
            windows = [3, 6, 12, 24]
            for w in windows:
                col = f'aqi_rolling_mean_{w}h'
                if col in new_row.index and len(aqi_values) >= w:
                    new_row[col] = float(np.mean(aqi_values[-w:]))

            if 'aqi_rolling_std_6h' in new_row.index and len(aqi_values) >= 6:
                new_row['aqi_rolling_std_6h'] = float(np.std(aqi_values[-6:]))

            if 'aqi_rolling_std_24h' in new_row.index and len(aqi_values) >= 24:
                new_row['aqi_rolling_std_24h'] = float(np.std(aqi_values[-24:]))

            if 'aqi_change_24h' in new_row.index and len(aqi_values) >= 24:
                new_row['aqi_change_24h'] = float(aqi_values[-1] - aqi_values[-24])
            ...
            if 'pm2_5_x_wind_speed' in new_row.index:
                new_row['pm2_5_x_wind_speed'] = new_row['pm2_5'] * new_row['wind_speed']
            ...
            # Append predicted AQI BEFORE next iteration reads lags
            aqi_values.append(pred_class)
            ...
            if hour_offset % 24 == 0:
                print(f"   Predicted {hour_offset} hours | last AQI: {pred_class}")
